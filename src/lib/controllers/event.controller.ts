...
// fetch the event
export const getEvent = asyncHandler(async (req: NextRequest, context: MiddlewareContext | undefined) => {
...
export const updateEvent = asyncHandler(async (req: NextRequest, context: MiddlewareContext | undefined) => {
    ...
    return NextResponse.json(new ApiResponse(200, { eventId: event?._id }, "Event updated"));

});

// remove the event
export const deleteEvent = asyncHandler(async (req: NextRequest, context: MiddlewareContext | undefined) => {
    const { searchParams } = new URL(req.url);
    const eventId = searchParams.get("id");
