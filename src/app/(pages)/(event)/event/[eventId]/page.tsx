...
export default function EventRegistration() {
    ...
    return (
        <div className='mx-auto '>
            {/* Image and details */}
            <div className={' p-10 mx-auto relative bg-cover bg-center h-[450px] mb-8'} style={{ backgroundImage: `url(${eventData?.thumbnail})` }}>
                ...
                <div className='absolute z-2 inset-0 space-y-8 flex flex-col justify-center p-10'>
                    ...
                    <div className='flex flex-col justify-c enter gap-3'>
                        <h2 className='text-white text-3xl md:text-4xl lg:text-6xl'>{eventData?.title}</h2>
                    ...
                    <div className='flex gap-4 md:gap-7 flex-wrap items-center text-white'>
                        {eventData?.date && <div className='flex justify-center items-center gap-2 py-2 px-4 rounded-lg bg-[#4f4f4f22] backdrop-blur-sm'>
                            <span className='text-green-400'><Calendar /></span>
                            <span>{formatDate(eventData.date)}</span>
                        </div>}
                        {eventData?.time && <div className='flex justify-center items-center gap-2 py-2 px-4 rounded-lg bg-[#4f4f4f22]  backdrop-blur-sm'>
                            <span className='text-orange-400'><Clock /></span>
                            <span>{formatDate(eventData?.time)}</span>
                        </div>}
                        {eventData?.location && <div className='flex justify-center items-center gap-2 py-2 px-4 rounded-lg bg-[#4f4f4f22]  backdrop-blur-sm'>
                            <span className='text-blue-400'><MapPin /></span>
                            <span>{formatDate(eventData?.location)}</span>
                        </div>}
                    </div>
                </div>
            </div>
            ...
            <div className='grid md:grid-cols-[70%_30%] gap-5 lg:gap-10 mx-auto max-sm:px-5 px-10 lg:px-20 my-12'>
                <div>
                    ...
                    <div className='rounded-xl border-2 border-gray-200 border-l-5 border-l-[var(--primary)] p-8 bg-gray-100'>
                        <pre className='text-lg whitespace-pre-wrap font-sans'>{eventData?.description}</pre>
                    </div>
                </div>
                {/* registration */}
                    ...
                    <div className='text-gray-600 border-t-2 border-[var(--background)] space-y-6 p-4'>
                        <div className='space-y-2'>
                            <div className='flex gap-2'>
                                Fee: <span className="font-semibold text-[var(--foreground)]">
                                    {eventData?.fee &&
                                        (eventData.fee === 0
                                            ? 'Free'
                                            : <span className='flex items-center'><IndianRupee size={15} />{eventData.fee}</span>)
                                    }
                                </span>
                            </div>
                            {eventData?.deadline && <div className='flex gap-2'>
                                Deadline: <span className="font-semibold text-[var(--foreground)]">{formatDate(eventData.deadline)}</span>
                            </div>}
                        </div>
                        {/* Regisration Button */}
